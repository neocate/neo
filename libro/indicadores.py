# ---------------------------------------------------------------------------
# indicadores.py - CVD, VWAP, POC y footprint sobre los CSV de datos/flujo/.
#
#   Lee trades_<COIN>_<MERCADO>_<fecha>.csv (tick a tick, con lado agresor)
#   y flujo_<COIN>_<MERCADO>_<fecha>.csv (agregado por minuto) para un rango
#   de dias, y calcula:
#
#     - CVD: recalculado desde los trades crudos ya deduplicados, comparado
#       contra el cvd que ya escribe flujo.py.
#     - VWAP de la sesion completa.
#     - POC: nivel de precio con mas volumen negociado (perfil de volumen).
#     - Footprint: desglose buy/sell por nivel de precio de una ventana de
#       1 minuto (por defecto, la de mas trades del rango).
#
#   OJO: en los dias sin columna id en trades_*.csv (anteriores al
#   2026-09-04) hay filas duplicadas exactas (5-8% del total, comprobado en
#   los datos: mismo timestamp_exchange_ms+precio+volumen+lado repetido).
#   El cvd/vol_buy/vol_sell/n_trades que flujo.py escribio para esos dias
#   esta calculado SOBRE esos duplicados. Este script deduplica antes de
#   recalcular, asi que en esos dias su cvd_recalc NO coincidira con el
#   cvd guardado en flujo_*.csv - la diferencia no es un fallo de este
#   script, es que el numero guardado en flujo_*.csv esta inflado.
#
# Uso:
#   python indicadores.py eth --desde 2026-09-03
#   python indicadores.py eth --desde 2026-09-01 --hasta 2026-09-03
#   python indicadores.py eth --desde 2026-09-03 --footprint "2026-09-03 14:50"
# ---------------------------------------------------------------
...
def _rutas_trades(datos, coin, mercado, fecha):
    """Normalmente un fichero por dia, pero un cambio de esquema a media
    jornada (ej. 2026-09-04, se anadio la columna id) puede dejar el dia
    partido en <base>.csv + <base>_pre_id.csv. Se cogen todas las
    variantes de esa fecha para no perder las horas de antes del cambio."""
...
def cargar_rango(datos, coin, mercado, desde, hasta):
    """Concatena trades_*.csv y flujo_*.csv de todos los dias del rango.
    Los dias sin fichero se saltan (huecos reales de recoleccion)."""
    ...
    for fecha in _rango_fechas(desde, hasta):
        ...
        if rutas_t and os.path.exists(rf):
            ...
            # dia del FICHERO, no de fecha_utc: la ultima ventana del dia
            # (23:59-00:00) lleva fecha_utc del dia siguiente a medianoche,
            # pero flujo.py la guarda y acumula el cvd en el fichero de hoy.
...
def _dedup_trades(trades):
    """Los dias sin columna id (anteriores al 2026-09-04) tienen filas
    duplicadas exactas por un fallo de flujo.py en el corte de ventana:
    la misma operacion se cuenta dos veces (hasta 8% de las filas en
    algunos dias). Se deduplica por id donde existe; donde no, por
...
def calcular_cvd(trades, flujo):
    """CVD recalculado desde trades crudos, comparado con el de flujo.py.
    flujo.py reinicia el cvd cada dia (un fichero por dia), asi que el
    cumsum se hace por dia para que la comparacion valga tambien en rangos
    de varios dias."""
...
def calcular_footprint(trades, flujo, bin_size=0.1, ventana_fin_ms=None):
    """Desglose buy/sell por nivel de precio en una ventana de 1 minuto.
    Sin ventana_fin_ms explicita, usa la de mas trades del rango cargado."""
